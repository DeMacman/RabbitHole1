...
export async function getEntityBySlug(
  ...
  try {
    const result = await session.run(
      'MATCH (e:Entity {slug: $slug}) RETURN e',
      { slug }
    );
...
export async function listEntities(
  ...
  try {
    ...
    query +=
      ' RETURN e ORDER BY e.label SKIP $offset LIMIT $limit';

    const result = await session.run(query, params);

    return result.records.map((rec: any) =>
      nodeToEntity(rec.get('e'))
    );
  } finally {
    session.close();
  }
...
export async function updateEntityBySlug(
  ...
  try {
    ...
    if (data.image !== undefined) {
      setClauses.push('e.image = $image');
      params.image = data.image;
    }

    if (data.banner !== undefined) {
      setClauses.push('e.banner = $banner');
      params.banner = data.banner;
    }

    if (data.wikipediaUrl !== undefined) {
      setClauses.push('e.wikipediaUrl = $wikipediaUrl');
      params.wikipediaUrl = data.wikipediaUrl;
    }

    if (data.officialWebsite !== undefined) {
      setClauses.push('e.officialWebsite = $officialWebsite');
      params.officialWebsite = data.officialWebsite;
    }

    if (data.socialLinks !== undefined) {
      setClauses.push('e.socialLinks = $socialLinks');
      params.socialLinks = JSON.stringify(data.socialLinks);
    }
...
export async function deleteEntityBySlug(
  slug: string
): Promise<boolean> {
  const session = getSession();
...
export async function searchEntities(
  ...
  try {
    ...
    try {
      ...
      results = result.records.map((rec: any) => ({
        entity: nodeToEntity(rec.get('node')),
        score: Number(rec.get('score')),
      }));
    } catch (e) {
      results = await basicSearch(
        session,
        q,
        type,
        limit,
...
async function basicSearch(
  ...
  let query = `
    MATCH (e:Entity)
    WHERE e.label CONTAINS $q
       OR any(alias IN e.aliases WHERE alias CONTAINS $q)
       OR any(tag IN e.tags WHERE tag CONTAINS $q)
  `;
  ...
  query +=
    ' RETURN e, 0.5 as score ORDER BY e.label SKIP $offset LIMIT $limit';
...
export async function getEntityGraph(
  ...
  try {
    ...
    const neighbourNodes: Entity[] = (
      record.get('neighbours') || []
    )
      .filter((n: any) => n !== null)
      .map(nodeToEntity);
    ...
    const edges: Relationship[] = (
      ...
      .map((e: any) => ({
        ...
        sourceReferences: safeParseJSON(
          e.sourceReferences,
          []
        ),
